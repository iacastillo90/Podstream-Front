<template>
  <div class="h-full">
    <div class="max-w-5xl mx-auto space-y-8 animate-fade-in-up">
      <h1 class="text-3xl md:text-4xl font-bold font-heading mb-6">Pedidos Pendientes</h1>

      <!-- Order List -->
      <div v-if="pendingOrders.length > 0" class="space-y-6">
        <GlassCard
        ...
        >
          ...
          >
            <div>
              <h2 class="text-xl font-bold text-white flex items-center gap-3">
                Pedido #{{ order.id }}
                <span
                  class="px-3 py-1 bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 rounded-full text-xs font-bold uppercase tracking-wider"
                >
                  {{ order.status }}
                </span>
              </h2>
          ...
          <div class="flex flex-wrap gap-3 justify-end">
            <button
              v-if="['shipped', 'delivered'].includes(order.status.toLowerCase())"
              class="px-4 py-2 rounded-lg bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/30 transition-colors text-sm font-bold flex items-center gap-2"
            ...
            <AnimatedButton class="!py-2 !px-6 text-sm" @click="contactSupport()">
              Contactar Soporte
            </AnimatedButton>
          </div>
        </GlassCard>
      ...
      <GlassCard v-else class="p-12 text-center">
        ...
        <h2 class="text-2xl font-bold font-heading mb-2">No tienes pedidos pendientes</h2>
        <p class="text-gray-400 mb-8 max-w-md mx-auto">
          Tus pedidos en curso aparecerán aquí. ¡Explora más productos mientras tanto!
        </p>
...
const confirmReceipt = async (orderId: number) => {
  ...
  try {
    ...
    // Reload to move order to completed list (or update status locally)
    await fetchOrders()

    // Prompt for review
    if (confirm('¡Pedido completado! ¿Te gustaría dejar una reseña de tus productos ahora?')) {
      // Redirect to the order details page or straight to a review flow if available.
      // Assuming Order Detail has the review capability or links to products.
      // Using confirm/cancel style for simplicity.
      router.push(`/orders/${orderId}`)
    }
  } catch (error) {
    console.error('Error confirming delivery', error)
    alert('Hubo un error al confirmar la recepción.')
  }
}
</script>
